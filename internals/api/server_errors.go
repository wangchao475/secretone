...
// Errors returned by the secretone API
var (
	...
	// GCP IdP
	ErrGCPServiceAccountAlreadyTaken = errHub.Code("gcp_service_account_taken").StatusError("a secretone service account coupled to that GCP Service Account email already exists. Delete the existing secretone service account or create a new one using a different GCP Service Account email.", http.StatusConflict)
	...
	ErrDirNotExist               = "dir dose not exist"
)

// IsErrNotFound returns whether the given error is caused by a un-existing resource.
func IsErrNotFound(err error) bool {
	var publicStatusError errio.HttpError
	ok := errors.As(err, &publicStatusError)
	if !ok {
		return false
	}
	if publicStatusError.Status == 404 && publicStatusError.Message != "no_secret_key_found_with_status_ok" {
		return true
	}
...
func IsNotFound(err error) bool {
	...
	if publicStatusError.Status == 404 {
		return true
	}
	return false
}
func IsErrNoDirExist(err error) bool {
	var publicStatusError errio.HttpError
	ok := errors.As(err, &publicStatusError)
	if !ok {
		return false
	}
	if publicStatusError.Status == 404 && publicStatusError.Message == ErrDirNotExist {
		return true
	}
	return false
}
func IsErrNotStatusOK(err error) bool {
	var publicStatusError errio.HttpError
	ok := errors.As(err, &publicStatusError)
	if !ok {
		return false
...
// IsKnownError returns whether the given error is a known secretone error.
