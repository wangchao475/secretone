...
func (es *EncryptedSecret) Decrypt(accountKey *crypto.SM2PrivateKey) (*Secret, error) {
...
// Secret represents a decrypted secret in secretone.
type Secret struct {
	SecretID uuid.UUID `json:"secret_id"`
	DirID    uuid.UUID `json:"dir_id"`
	RepoID   uuid.UUID `json:"repo_id"`
...
type SecretKeyMemberRequest struct {
	...
	EncryptedKey crypto.CiphertextSM2 `json:"encrypted_key"`
}
