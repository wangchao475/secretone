...
func (t EncryptedTree) DecryptContents(accountKey *crypto.SM2PrivateKey) ([]*Dir, []*Secret, error) {
...
func (t EncryptedTree) Decrypt(accountKey *crypto.SM2PrivateKey) (*Tree, error) {
...
func (t Tree) AbsDirPath(dirID uuid.UUID) (DirPath, error) {
	if uuid.Equal(dirID, t.RootDir.DirID) {
		if t.RootDir.Name == "" { //根目录的名称为空，直接返回path即可
			dirPath, _ := NewDirPath(t.RootDir.Path)
			return dirPath, nil
		}
