...
func TestValidateGCPServiceAccountEmail(t *testing.T) {
	cases := map[string]struct {
		...
		"user  managed service account": {
			in: "test-service-account@secretone-test-1234567890.iam.gserviceaccount.com",
		},
		"appspot service account": {
			in:        "secretone-1234567890@appspot.gserviceaccount.com",
			expectErr: true,
		},
		"compute service account": {
			in:        "secretone-1234567890-compute@developer.gserviceaccount.com",
			expectErr: true,
		},
		"google managed service account": {
			in:        "secretone-1234567890@cloudservices.gserviceaccount.com",
			expectErr: true,
		},
		"not an email": {
		...
		"non-service account email": {
			in:        "serviceaccount@secretone.io",
			expectErr: true,
		},
		"empty string": {
			in:        "",
...
func TestProjectIDFromGCPEmail(t *testing.T) {
	cases := map[string]struct {
		...
		"user  managed service account": {
			in:     "test-service-account@secretone-test-1234567890.iam.gserviceaccount.com",
			expect: "secretone-test-1234567890",
		},
		"invalid email": {
			in:        "secretone-1234567890-compute@developer.gserviceaccount.com",
			expectErr: true,
...
func TestValidateGCPKMSKeyResourceID(t *testing.T) {
	cases := map[string]struct {
		...
		"valid": {
			in: "projects/secretone-test-1234567890/locations/global/keyRings/test/cryptoKeys/test",
		},
		"kerying only": {
			in:        "projects/secretone-test-1234567890/locations/global/keyRings/test",
			expectErr: true,
		},
		"too many segment": {
			in:        "projects/secretone-test-1234567890/locations/global/keyRings/test/cryptoKeys/test/extrasegment",
			expectErr: true,
		},
		"empty string": {
			in:        "",
		...
		"missing required path segment": {
			in:        "projects/secretone-test-1234567890//global/keyRings/test/cryptoKeys/test",
			expectErr: true,
		},
		"wrong required path segment": {
			in:        "projects/secretone-test-1234567890/locations/global/thingiswrong/test/cryptoKeys/test",
			expectErr: true,
		},
		"leading slash": {
			in:        "/projects/secretone-test-1234567890/locations/global/keyRings/test/cryptoKeys/test",
