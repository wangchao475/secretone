...
// Errors
var (
	ErrInvalidCipher = errCrypto.Code("sm4_cipher_invalid").ErrorPref("cipher is invalid: %v")
	ErrSM4Decrypt    = errCrypto.Code("sm4_decrypt_failed").ErrorPref("could not decrypt data: %s")
	ErrSM4Encrypt    = errCrypto.Code("sm4_encrypt_failed").ErrorPref("could not encrypt data: %s")
	ErrAESEncrypt    = errCrypto.Code("aes_encrypt_failed").ErrorPref("could not encrypt data: %s")
	ErrAESDecrypt    = errCrypto.Code("aes_encrypt_failed").ErrorPref("could not encrypt data: %s")
)
...
const (
	...
	SymmetricKeyLength = sm4.BlockSize // 16
)
...
func (k *SymmetricKey) RSAEncrypt(data []byte) (CiphertextAES, error) {
...
// Encrypt uses the key to encrypt given data with the sm4 algorithm,
// returning the resulting ciphertext.
func (k *SymmetricKey) Encrypt(data []byte) (CiphertextSM4, error) {

	encData, err := Sm4Encrypt(k.key, data)
	if err != nil {
		return CiphertextSM4{}, ErrSM4Encrypt(err)
	}
...
func (k *SymmetricKey) RSADecrypt(ciphertext CiphertextAES) ([]byte, error) {
...
func (k *SymmetricKey) Decrypt(ciphertext CiphertextSM4) ([]byte, error) {

	output, err := Sm4Decrypt(k.key, ciphertext.Data)
	if err != nil {
		return nil, ErrSM4Decrypt(err)
	}
	return output, nil
}

// HMAC uses the key to create a Hash-based Message Authentication Code of the
// given data with the sm3 hashing algorithm, returning the given hash bytes.
func (k SymmetricKey) HMAC(data []byte) ([]byte, error) {
	//mac := hmac.New(sha256.New, k.key)
	mac := hmac.New(sm3.New, k.key)
