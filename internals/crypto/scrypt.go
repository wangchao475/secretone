...
//	}
//
//	return k.key.RSADecrypt(ciphertext)
//}

func (k *ScryptKey) Decrypt(ciphertext CiphertextSM4, operation SaltOperation) ([]byte, error) {
	err := k.Salt.Purpose().Verify(k.KeyLen, "aesgcm", operation)
	if err != nil {
		return nil, errio.Error(err)
	}
...
//	}
//
//	ciphertext, err := k.key.RSAEncrypt(data)
//	if err != nil {
...
//}
func (k *ScryptKey) Encrypt(data []byte, operation SaltOperation) (CiphertextSM4, error) {
	err := k.Salt.Purpose().Verify(k.KeyLen, "aesgcm", operation)
	if err != nil {
