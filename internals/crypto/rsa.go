...
var (
	...
	ErrGenerateSM2Key                  = errCrypto.Code("sm2_generate_fail").Error("could not generate SM2 key")
	...
	ErrSM2Encrypt                      = errCrypto.Code("sm2_encrypt_failed").ErrorPref("could not encrypt data: %s")
	...
	ErrNotSM2Format                    = errCrypto.Code("key_wrong_format").Error("key must be in sm2 pkcs8 format")
...
func (pub RSAPublicKey) Encrypt(data []byte) (CiphertextRSAAES, error) {
	...
	aesData, err := aesKey.RSAEncrypt(data)
	if err != nil {
		return CiphertextRSAAES{}, errio.Error(err)
	}
...
func (prv RSAPrivateKey) Decrypt(ciphertext CiphertextRSAAES) ([]byte, error) {
	...
	return NewSymmetricKey(aesKeyData).RSADecrypt(ciphertext.AES)
}
...
// the sm2 algorithm, returning the resulting decrypted bytes. Note that this
...
// UnwrapBytes uses the private key to decrypt a small number of encrypted bytes with
// the sm2-sm4 algorithm, returning the resulting decrypted bytes. Note that this
// function will be deprecated. Directly use Unwrap instead when possible.
func (prv SM2PrivateKey) UnwrapBytes(encryptedData []byte) ([]byte, error) {
	return prv.unwrap(encryptedData)
}
