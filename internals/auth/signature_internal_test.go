...
func TestGetMessage_Post(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "https://api.secretone.io/repos/jdoe/catpictures", body)
	assert.OK(t, err)
	req.Header.Set("Date", "Fri, 10 Mar 2017 16:25:54 CET")
...
func TestGetPayloadToSign_ContentLength(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "https://api.secretone.io/repos/jdoe/catpictures", requestBody)
	assert.OK(t, err)
	req.Header.Set("Date", "Fri, 10 Mar 2017 16:25:54 CET")
