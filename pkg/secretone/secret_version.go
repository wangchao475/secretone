...
var (
	...
	ErrCannotReWriteToVersion = errClient.Code("cannot_re_write_version").Error("cannot (over)write the specific secret version, please user write command to create a new version")
)
...
// SecretVersionService handles operations on secret versions from secretone.
...
func (s secretVersionService) Delete(path string) error {
	...
	err = s.client.httpClient.DeleteSecretVersion(secretPath.String(), version)
	if err != nil {
		return errio.Error(err)
	}
...
func (s secretVersionService) get(path api.SecretPath, withData bool) (*api.SecretVersion, error) {
	...
	if path.HasVersion() {
		...
		//versionParam = "latest"
		//如果不写版本号，改成默认的，最新的需要用户手动加:latest
		versionParam = "default"
	}

	encVersion, err := s.client.httpClient.GetSecretVersion(path.String(), versionParam, withData)
	if api.IsErrNotFound(err) {
		return nil, &errSecretNotFound{path: path, err: err}
	} else if err != nil {
		return nil, errio.Error(err)
...
func (s secretVersionService) list(path api.SecretPath, withData bool) ([]*api.SecretVersion, error) {
	...
	versions, err := s.client.httpClient.ListSecretVersions(path.String(), withData)
	if api.IsErrNotFound(err) {
		return nil, &errSecretNotFound{path: path, err: err}
	} else if err != nil {
		return nil, errio.Error(err)
...
func (c *Client) createSecretVersion(secretPath api.SecretPath, data []byte, secretKey *api.SecretKey) (*api.SecretVersion, error) {
	...
	resp, err := c.httpClient.CreateSecretVersion(secretPath.String(), in)
	if err != nil {
		return nil, errio.Error(err)
	}
	...
	for {
		...
		for _, account := range accounts {
			...
			if !ok {
				...
				encryptedName, err := setNameForAccount(secretName, account)
				if err != nil {
					return nil, err
				}
		...
		resp, err := c.httpClient.CreateSecret(secretPath.GetNamespace(), secretPath.GetRepo(), parentPath.String(), in)
		if err == nil {
			accountKey, err := c.getAccountKey()
			if err != nil {
				return nil, err
