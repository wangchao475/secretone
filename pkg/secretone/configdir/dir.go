// Package configdir provides simple functions to manage the secretone
...
// Default is the default way to get the location of the secretone
// configuration directory, sourcing it from the environment variable
// secretone_CONFIG_DIR or falling back to the ~/.secretone directory.
func Default() (*Dir, error) {
	envDir := os.Getenv("SECRETONE_CONFIG_DIR")
	if envDir != "" {
		return &Dir{
			path: envDir,
		}, nil
	...
	return &Dir{
		path: filepath.Join(homeDir, ".secretone"),
	}, nil
}

// Credential returns the file that contains the secretone API credential.
...
// CredentialFile represents the file that contains the secretone API credential.
...
func (f *CredentialFile)Check() (bool,error){  //没什么作用，为了复用接口
	return false,nil
}
