...
// AccessRuleService handles operations on access rules from secretone.
...
func (s accessRuleService) Delete(path string, accountName string) error {
	...
	err = s.client.httpClient.DeleteAccessRule(p.String(), an)
	if err != nil {
		return errio.Error(err)
	}
...
func (s accessRuleService) Get(path string, accountName string) (*api.AccessRule, error) {
	...
	accessRule, err := s.client.httpClient.GetAccessRule(p.String(), an)
	if err != nil {
		return nil, errio.Error(err)
	}
...
func (s accessRuleService) List(path string, depth int, ancestors bool) ([]*api.AccessRule, error) {
	...
	rules, err := s.client.httpClient.ListAccessRules(p.String(), depth, ancestors)
	if err != nil {
		return nil, errio.Error(err)
	}
...
func (s accessRuleService) ListLevels(path string) ([]*api.AccessLevel, error) {
	...
	rules, err := s.client.httpClient.ListAccessRules(p.String(), 0, true)
	if err != nil {
		return nil, errio.Error(err)
	}
...
func (s accessRuleService) Set(path string, permission string, accountName string) (*api.AccessRule, error) {
	...
	return s.update(p.String(), perm, an)
}

// CreateAccessRule creates a new AccessRule for an account with a certain permission level.
//原来path类型为api.BlindNamePath
func (s accessRuleService) create(path api.DirPath, permission api.Permission, accountName api.AccountName) (*api.AccessRule, error) {
	...
	for {
		//解析当前权限
		permissionArray := strings.Split(currentAccessLevel.Permission, ",")
		if len(permissionArray) == 1 { //之前没有授权过，需要添加路径
			if permissionArray[0] == api.PermissionNone {
				err = encrypter.Add(blindName.String())
				if err != nil {
					return nil, err
				}
		...
		accessRule, err := s.client.httpClient.CreateAccessRule(blindName.String(), accountName, in)
		if err == nil {
			return accessRule, nil
		}
...
func (re *reencrypter) Add(blindName string) error {
	encryptedTree, err := re.client.httpClient.GetTree(blindName, -1, false)
	if err != nil {
		return err
	}
...
func (re *reencrypter) reencrypt(encryptedTree *api.EncryptedTree, accountKey *crypto.SM2PrivateKey) error {
	for _, dir := range encryptedTree.Directories {
		_, ok := re.dirs[dir.DirID]
		if !ok {
			//这里没有加密，就不解密,函数里面去掉了加解密过程，只保留数据
	...
	for _, secret := range encryptedTree.Secrets {
		...
		if !ok {
			...
			//	return err
			//}
			//这里不是解密秘钥，而是解密路径，因为路径没有加密了，所以去掉加解密过程
			encrypted, err := re.client.encryptSecretFor(secret, re.encryptFor)
...
func (s accessRuleService) update(path string, permission api.Permission, name api.AccountName) (*api.AccessRule, error) {
	//blindName, err := s.client.convertPathToBlindName(path)
	//if err != nil {
	//	return nil, errio.Error(err)
	//}
	...
	accessRule, err := s.client.httpClient.UpdateAccessRule(path, name, in)
...
//}
func (c *Client) getAccessLevel(path api.DirPath, accountName api.AccountName) (*api.AccessLevel, error) {
	//blindName, err := c.convertPathToBlindName(path)
	//if err != nil {
	//	return nil, errio.Error(err)
	//}

	accessLevel, err := c.httpClient.GetAccessLevel(path.String(), accountName)
	if err != nil {
		return nil, errio.Error(err)
	}
