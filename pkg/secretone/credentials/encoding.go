...
var (
	...
	ErrNeedPassphrase                    = errCredentials.Code("credential_passphrase_required").Error("credential is password-protected. Configure a credential passphrase through the SECRETONE_CREDENTIAL_PASSPHRASE environment variable or use a credential that is not password-protected")
...
var (
	...
	DefaultDecoders = []Decoder{sm2PrivateKeyDecoder{}}
...
func (c encodedCredential) Decode() (*SM2Credential, error) {
	if c.IsEncrypted() {
...
// using the given key.
func (c encodedCredential) DecodeEncrypted(key PassBasedKey) (*SM2Credential, error) {
...
// rsaPrivateKeyDecoder implements the Decoder interface for an RSA private key.
type sm2PrivateKeyDecoder struct{}

// Decode converts a encodedCredential's payload into an RSA ClientKey.
func (d sm2PrivateKeyDecoder) Decode(payload []byte) (*SM2Credential, error) {
	key, err := x509.ParsePKCS8UnecryptedPrivateKey(payload)
	if err != nil {
		return nil, ErrMalformedCredential(err)
	}
	return &SM2Credential{
		crypto.NewSM2PrivateKey(key),
	}, nil
}

// Name returns the encoding name.
func (d sm2PrivateKeyDecoder) Name() string {
	return "sm2"
}
...
type Decoder interface {
	...
	Decode(payload []byte) (*SM2Credential, error)
