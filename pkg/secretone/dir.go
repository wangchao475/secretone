...
import (
	...
	"github.com/wangchao475/secretone/pkg/secretpath"
)

// DirService handles operations on directories from secretone.
...
//如果返回全部信息即depth=-1，全部信息里包含了根的信息
//如果depth > 0 ,后台会在原有层级上+1返回，因为本身根占用了一级
func (s dirService) GetTree(path string, depth int, ancestors bool) (*api.Tree, error) {
	...
	encTree, err := s.client.httpClient.GetTree(path, depth, ancestors)
	if err != nil {
		return nil, errio.Error(err)
	}
	//
...
func (s dirService) Create(path string) (*api.Dir, error) {
	...
	tries := 0
	for {
		accounts, err := s.client.listDirAccounts(parentPath.String())
		if err != nil {
			return nil, errio.Error(err)
		}
		...
		for _, account := range accounts {
			...
			if !ok {
				...
				encryptedName, err := setNameForAccount(dirName, account)
				if err != nil {
					return nil, err
				}
		...
		request := &api.CreateDirRequest{
			PathName:        blindName.String(),
			ParentBlindName: parentBlindName.String(),
		...
		if err == nil {
			...
			//}
			//不需要解密
			dir, err := encryptedDir.Decrypt(nil)
...
func (s dirService) Delete(path string) error {
	...
	err = s.client.httpClient.DeleteDir(p.String())
	if err != nil {
		return errio.Error(err)
	}
...
func (c *Client) listDirAccounts(path string) ([]*api.Account, error) {
	//blindName, err := c.convertPathToBlindName(path)
	//if err != nil {
	//	return nil, errio.Error(err)
	//}

	accounts, err := c.httpClient.ListDirAccounts(path)
